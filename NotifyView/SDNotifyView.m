...
@interface SDNotifyView ()<CAAnimationDelegate>
/** mainView */
@property (nonatomic, strong) UIView *mainView;
/** label */
@property (nonatomic, strong) UILabel *label;
/** config */
@property (nonatomic, strong) SDNotifyViewConfig *config;
...
+ (SDNotifyView *)showNotify:(NSString *)notify showView:(UIView *)showView {
    return [self showNotify:notify showView:showView config:[SDNotifyViewConfig defaultNotifyConfig]];
}

+ (SDNotifyView *)showNotify:(NSString *)notify showView:(UIView *)showView config:(SDNotifyViewConfig *)config {
    if (!config) {
        config = [SDNotifyViewConfig defaultNotifyConfig];
    }
    if (!showView) {
...
- (SDNotifyView *)initWithNotify:(NSString *)notify showView:(UIView *)showView config:(SDNotifyViewConfig *)config{
    if (self = [super init]) {
        self.notify = notify;
        self.config = config;
...
- (void)initNotifyAnimation {
    ...
    if (@available(iOS 9.0, *)) {
        ...
        if (self.config.notifyStyle == SDNotifyViewStyleFill) {
        ...
        } else if (self.config.notifyStyle == SDNotifyViewStyleFit) {
...
- (void)initNotifyUI {
    ...
    if (self.config.notifyStyle == SDNotifyViewStyleFit) {
...
@implementation UIColor (SDNotifyView)
...
@implementation SDNotifyViewConfig
...
- (void)initNotifyConfig {
    self.notifyStyle = SDNotifyViewStyleFit;
    

    self.backgroundColorType = SDNotifyViewBackgroundColorTypeSuccess;
